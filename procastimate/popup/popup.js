...
function tick(){
  if (remainingSeconds > 0) {
    ...
    if (mode === "work") {
      ...
      chrome.notifications.create({type:"basic",iconUrl:"../assets/icons/128.png",title:"Break time!",message:"Take a short break."});
    } else {
      mode = "work";
      remainingSeconds = Number(workMinsInput.value) * 60;
      chrome.notifications.create({type:"basic",iconUrl:"../assets/icons/128.png",title:"Back to work",message:"Start your next Pomodoro."});
    }
    updateDisplay();
    chrome.tabs.query({}, (tabs) => { for (const t of tabs) chrome.tabs.sendMessage(t.id, {type:"timer_update"}); });
  }
}
...
startBtn.addEventListener("click", () => { startTimer(); saveSettings(); });
pauseBtn.addEventListener("click", () => { pauseTimer(); saveSettings(); });
resetBtn.addEventListener("click", () => { resetTimer(); saveSettings(); });
workMinsInput.addEventListener("change", () => { remainingSeconds = Number(workMinsInput.value) * 60; saveSettings(); updateDisplay(); });
breakMinsInput.addEventListener("change", () => { saveSettings(); });
blockEnabled.addEventListener("change", () => {
  chrome.storage.local.set({ blockEnabled: blockEnabled.checked });
  chrome.tabs.query({}, (tabs) => { for (const t of tabs) chrome.tabs.sendMessage(t.id, {type: "update_block_status"}); });
});
